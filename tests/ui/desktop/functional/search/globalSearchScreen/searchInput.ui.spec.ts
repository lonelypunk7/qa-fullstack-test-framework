...
test("@TID-SEARCH-013 Тап на саджест при введенном поисковом запросе на странице поиска", async ({
  page,
}) => {
  const objectMocker = new ObjectMocker(page);
...
test("@TID-SEARCH-014 Ввод поискового запроса в поисковую строку на странице поиска", async ({
  page,
}) => {
  const objectMocker = new ObjectMocker(page);
  const suggestRoot = searchPom.suggest.root;
